...
class DataTransaksiController extends GetxController {
  ...
  Future<void> generateAndOpenInvoicePDF(List<Map<String, dynamic>> data) async {
    ...
    final file = File(
        '${output.path}/Data_Transaksi_Periode${formatDate(startDate.toString())}-${formatDate(endDate.toString())}.pdf');
  ...
  Future<void> generateAndOpenInvoicePDFSimple(List<Map<String, dynamic>> data) async {
    final randomInvoiceNumber = Random().nextInt(99999) + 10000;
    await getDataKaryawan();
    if (kDebugMode) {
      print('Nama Karyawan setelah getDataKaryawan(): ${namaKaryawanC.text}');
    ...
    final output = await getTemporaryDirectory();
    final file = File(
        '${output.path}/Data_Transaksi_Simple_Periode${formatDate(startDate.toString())}-${formatDate(endDate.toString())}.pdf');
    await file.writeAsBytes(await pdf.save());
